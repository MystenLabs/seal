...
- To support future upgrades and maintain backward compatibility, define `seal_approve*` functions as non-public `entry` functions whenever possible. Also, version your shared objects or use a shared global object with the latest version (see the [whitelist](./move/patterns/sources/whitelist.move) and [subscription](./move/patterns/sources/subscription.move) examples).
