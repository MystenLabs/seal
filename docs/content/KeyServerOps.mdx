...
This guide covers how to set up and operate an **Independent server type** key server. For an overview of committee vs independent server types, see [Seal Server Overview](/ServerOverview).
...
- Initialize with empty client configs (clients can be added later).
```yaml
