...
# Key server operations for committee server type
...
Committee server type uses distributed key generation (DKG) to split cryptographic keys across multiple operators. In addition to the committee member key servers, an **aggregator server** is required to collect partial key shares from members and combine them into usable decryption keys for clients. For aggregator setup, see [Aggregator Server](/Aggregator).

**What you'll learn:**
- Running a fresh DKG to initialize a new committee and generate an initial key share
- Configuring and starting a committee member key server
- Performing key rotation to update committee membership while maintaining the same public key
- Updating a running key server to use the new key share after rotation

**How DKG works:**

The DKG process involves two roles: a **coordinator** orchestrates the workflow, and **committee members** participate in key generation and rotation. Both fresh DKG and key rotation follow the same three-phase process:
...
8. **Collect API keys for aggregator server**
