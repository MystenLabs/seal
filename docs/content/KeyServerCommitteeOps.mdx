...
- The committee state from *Active* to **Rotation** mode.
- Set the target committee version to `X+1` (increment 1 from the current version `X`). If you are not sure what `X` is in this rotation, check with your cooridinator.
