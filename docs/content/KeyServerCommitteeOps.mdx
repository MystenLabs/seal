...
If your Sui RPC endpoint requires API key authentication (common for gRPC connections to hosted Sui RPC providers), set the `SUI_RPC_API_KEY` environment variable when starting the key server:
```bash
CONFIG_PATH=crates/key-server/key-server-config.yaml MASTER_SHARE_V0=0x... SUI_RPC_API_KEY=<YOUR_SUI_RPC_API_KEY> cargo run --bin key-server
```
...
If your Sui RPC endpoint requires API key authentication, include the `SUI_RPC_API_KEY` environment variable:
```bash
CONFIG_PATH=crates/key-server/key-server-config.yaml \
  MASTER_SHARE_VX=<MASTER_SHARE_VX> \
  MASTER_SHARE_VX+1=<MASTER_SHARE_VX+1> \
  SUI_RPC_API_KEY=<YOUR_SUI_RPC_API_KEY> \
  cargo run --bin key-server
```
