...
### Decentralized key server and DKG

When running in committee mode, the key server uses a Distributed Key Generation (DKG) protocol to split the master secret key across multiple participants. During the DKG process, each participant signs their DKG messages with a payload that includes a domain separator and the `committee_id`. The domain separator prevents cross-protocol attacks by ensuring that DKG messages cannot be replayed in a different context, and the `committee_id` binds each message to a specific committee instance. Together, these measures ensure that DKG messages are authenticated and scoped to the intended committee, strengthening the security of the key generation process.
