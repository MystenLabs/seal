...
- **What you need**: Committee key server object ID + aggregator URL
- **Benefits**: The committee has its own internal threshold (e.g., 3-of-5 members), so you get distributed trust without managing multiple server relationships
...
- **What you need**: Committee key server object ID + aggregator URL + independent server object IDs (e.g., a 2-of-3 threshold with one committee server and two independent servers)
...
**Example - Hybrid (committee + independent):**
```typescript
const suiClient = new SuiClient({ url: getFullnodeUrl('testnet') });
