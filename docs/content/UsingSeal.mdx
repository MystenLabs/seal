...
=======
Read the [Seal Design document](/docs/Design) first to understand the underlying architecture and concepts before using this guide.
>>>>>>> 59efe92 (merge main)
...
=======
See [Example patterns](/docs/ExamplePatterns) for additional examples and high-level patterns.
>>>>>>> 59efe92 (merge main)
...
=======
Apps can define a list of Seal key server object IDs from the [verified key servers](/docs/Pricing#verified-key-servers) available in each environment. You can use any `Open` mode key server directly. For a `Permissioned` mode key server, contact the key server provider to register your access policy package ID and receive the corresponding object ID.
>>>>>>> 59efe92 (merge main)
...
=======
- The ID associated with the access control policy (without the prefix of the package ID discussed in [Seal Design](/docs/Design))
>>>>>>> 59efe92 (merge main)
- The data to encrypt
...
=======
Envelope encryption is also recommended for highly sensitive data and enables safer key rotation and updates without re-encrypting large blobs. See [Use layered encryption for critical or large data](/docs/SecurityBestPractices#use-layered-encryption-for-critical-or-large-data).
>>>>>>> 59efe92 (merge main)
