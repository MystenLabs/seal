...
- The setup requires an additional aggregator server component for client communication.

Learn more about [[key server operations for committee server types](https://claude.ai/KeyServerCommitteeOps)](/KeyServerCommitteeOps).
...
### Key servers (committee and independent)

**Requirements:** 
- Trusted access to a Sui full node for policy validation
- HTTPS/TLS exposure (ideally behind an API gateway or reverse proxy)
- Rate limiting enforced at the gateway
- Optional authentication via API keys or access tokens
- Monitoring with Prometheus metrics on port `9184` and a health check endpoint on port `2024`
- Secure key storage using a KMS or hardware/software vault

### Aggregator servers (committee only)

**Properties:** 
- Trustless component (anyone can run one)
- No key storage required
- Coordinates with committee member key servers to collect and combine partial key responses
