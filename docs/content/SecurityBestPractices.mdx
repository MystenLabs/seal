...
```note
Threshold configuration does not apply to decentralized (committee mode) key servers.
```
...
```note
This is only be applicable to decentralized key servers if you are planning to set up [your own aggregator](/Aggregator) and need API keys from key server providers.
```
