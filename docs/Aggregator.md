...
```shell
$ CONFIG_PATH=crates/key-server/aggregator-config.yaml cargo run --bin aggregator-server
```
...
$ docker run -p 2024:2024 \
  -v $(pwd)/crates/key-server/aggregator-config.yaml:/config/aggregator-config.yaml \
