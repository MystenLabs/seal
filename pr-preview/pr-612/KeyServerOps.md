...
```
6. To use the SDK against your localnet, configure dApp Kit's `createClient` with a localnet gRPC client (e.g. `new SuiGrpcClient({ network: 'localnet', baseUrl: 'http://127.0.0.1:9000' })`) and list your localnet key server in `serverConfigs`. See [Using Seal](./UsingSeal.mdx) for the full client flow.
```ts
