...
Verify each member's partial public key against the onchain value before trusting their key server. Fetch the onchain partial public keys for the committee:
```bash
cargo run --bin seal-committee-cli -- check-committee
```

For each member, confirm that the `partial_pk` returned in their curl response matches the onchain partial public key for their address. If any member's value does not match, resolve it with that member before proceeding.

Once all partial public keys are verified, collect and share the following with the aggregator operator:
...
After your key server is running successfully, generate an **API key name** and **API key** for your key server. These authenticate requests to your key server. 

7. **Verify your partial public key and share credentials**

Query the committee debug endpoint using the API credentials you just generated:
```bash
...
Send the coordinator the full curl request and response (which include your API key name and key) to the coordinator. 
