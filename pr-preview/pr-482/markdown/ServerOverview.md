...
**Open committee deployment**
- Any operator group can coordinate and deploy their own committee.
- Operators participate in a DKG ceremony and run their key servers with the DKG finalized key shares.
- Membership can be added, removed, or replaced through coordinated DKG rotations.
- The onchain object ID and public key remain unchanged, so clients are unaware of membership changes and existing encrypted data and configurations remain valid.

**Aggregator-mediated architecture**
- Clients send requests to an aggregator, which fans out to committee key servers and returns a single aggregated response.
- Committee members return partial responses encrypted under the user's public key.
- The aggregator is trust-minimized and cannot read key material or decrypt on its own.
...
An independent server is registered with one object ID and one server URL. Clients configure the threshold and communicate directly with the key servers. Encryption is tied to the specific set of key servers and threshold — changing key server membership or the threshold configuration requires re-encryption of existing data.
