...
- Notify members to begin **Phase B (Message creation)**.
- Monitor the off-chain storage (e.g. group chat) until continuing members have uploaded exactly the old committee threshold number of DKG message files. New members run `init-state` and do not upload message files.
