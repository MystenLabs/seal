...
7. Any party can call `propose_committee` to submit the partial public keys and key server public key. Committee object is transitioned to `State::PostDKG`.

8. Other party can call `approve_committee` if the submitted matches with what is finalized locally for the party. 

9. Any party can call `finalize_committee`. If the threshold is met, 
the `KeyServer` object and `PartialKeyServer` objects are created. Committee object is transitioned to `State::Finalized`.
...
b. A threshold of parties approve the committee after checking the onchain partial public keys match their local ones after DKG finalization.
...
6. Any member can now call `propose_committee` to submit the new partial pks.
7. All members can call `approve_committee` if the partial pks are consistent with their locally derived ones. 
8. Any member can call `finalize_committee`. If the threshold is met, the old committee is destroyed, the new committee is finalized. The key server object is transferred to the new committee, and all the partial key servers are created with new partial public keys. 
9. All members can now update their own partial key server's url. 
...
6. Any member can propose all partial pks and key server pk for rotation. 
...
7. A threshold of members need to approve the committee after confirming the partial pks are consistent with their locally derived ones. 

```shell
sui client switch --address $ADDRESS_0
sui client call --package $COMMITTEE_PKG --module committee \
  --function approve_committee \
  --args $NEW_COMMITTEE_ID
...
sui client call --package $COMMITTEE_PKG --module committee \
  ...
  --args $NEW_COMMITTEE_ID
```
8. Any member can now finalize the committee. 
