...
pub struct Committee {
    ...
    threshold: u16,
    pub members: Vec<sui_types::base_types::SuiAddress>,
...
/// Returns (members, candidate_addresses, candidate_public_keys)
pub async fn fetch_committee_candidate_data(
    ...
    // Extract members
    let members: Vec<String> = committee
        .members
        .iter()
        .map(|addr| format!("0x{}", hex::encode(addr.to_vec())))
        .collect();
    ...
    let candidate_data = match committee.state {
        ...
        CommitteeState::PostDKG { candidate_data, .. } => candidate_data,
    ...
    // Sort candidates by address to ensure consistent ordering
    let mut sorted_entries: Vec<_> = candidate_data.0.contents.iter().collect();
    sorted_entries.sort_by_key(|entry| entry.key);

    // Extract addresses and public keys in sorted order
    let mut candidate_addresses = Vec::new();
    let mut candidate_enc_pks = Vec::new();

    for entry in sorted_entries {
        let address = format!("0x{}", hex::encode(entry.key.to_vec()));
        ...
        candidate_enc_pks.push(enc_pk_hex);
    }

    Ok((members, candidate_addresses, candidate_enc_pks))
}
...
mod tests {
    ...
    async fn test_fetch_old_partial_pks_from_keyserver() {
        ...
        // Fetch partial PKs
        let result =
            fetch_old_partial_pks_from_keyserver(key_server_id, Client::TESTNET_FULLNODE).await;

        assert!(
            result.is_ok(),
            "Failed to fetch partial PKs: {:?}",
            result.err()
        );

        let partial_pks = result.unwrap();

        // Should have 4 parties (0, 1, 2, 3) based on onchain data
        assert_eq!(partial_pks.len(), 4, "Expected 4 partial PKs");

        // Verify all party IDs are present
        assert!(partial_pks.contains_key(&0), "Missing party 0");
        assert!(partial_pks.contains_key(&1), "Missing party 1");
        assert!(partial_pks.contains_key(&2), "Missing party 2");
        assert!(partial_pks.contains_key(&3), "Missing party 3");

        // Verify partial PKs match expected values from onchain KeyServer
        // The onchain data shows:
        // - Party 0: owner 0xe6a37ff5cd968b6a666fb033d85eabc674449f44f9fc2b600e55e27354211ed6
        // - Party 1: owner 0x0636157e9d013585ff473b3b378499ac2f1d207ed07d70e2cd815711725bca9d
        // - Party 2: owner 0x2aaadc85d1013bde04e7bff32aceaa03201627e43e3e3dd0b30521486b5c34cb
        // - Party 3: owner 0x8b4a608c002d969d29f1dd84bc8ac13e6c2481d6de45718e606cfc4450723ec2
        let expected_pks = [
        ...
        println!(
            "✓ Successfully fetched and verified {} partial public keys",
            partial_pks.len()
        );
    }
    ...
    async fn test_fetch_committee_candidate_data() {
        ...
        let result = fetch_committee_candidate_data(committee_id, Client::TESTNET_FULLNODE).await;

        assert!(
            result.is_ok(),
            "Failed to fetch committee data: {:?}",
            result.err()
        );

        let (members, candidate_addresses, candidate_enc_pks) = result.unwrap();

        // Verify we have the expected number of members and candidates
        println!("Members: {:?}", members);
        println!("Candidate addresses: {:?}", candidate_addresses);
        println!("Candidate enc_pks count: {}", candidate_enc_pks.len());

        // Verify expected values based on your provided data
        assert_eq!(members.len(), 3, "Expected 3 members");
        assert_eq!(candidate_addresses.len(), 3, "Expected 3 candidates");
        assert_eq!(
            candidate_enc_pks.len(),
            3,
            "Expected 3 encryption public keys"
        );

        // Create map of address -> enc_pk for easier verification
        let mut address_to_enc_pk: HashMap<String, String> = HashMap::new();
        for (i, addr) in candidate_addresses.iter().enumerate() {
            address_to_enc_pk.insert(addr.clone(), candidate_enc_pks[i].clone());
        }
        ...
        for (addr, expected_enc_pk) in &expected_mappings {
            assert_eq!(
                address_to_enc_pk.get(*addr).unwrap(),
                expected_enc_pk,
                "Address {} enc_pk mismatch",
                addr
            );
        }

        // Verify enc_pks can be decoded as valid PublicKeys
        for (i, enc_pk) in candidate_enc_pks.iter().enumerate() {
            let pk_bytes =
                Hex::decode(enc_pk).unwrap_or_else(|_| panic!("Failed to decode enc_pk {}", i));
            let _: PublicKey<G2Element> = bcs::from_bytes(&pk_bytes)
                .unwrap_or_else(|_| panic!("Failed to deserialize enc_pk {} as PublicKey", i));
        }

        println!("✓ Successfully fetched and verified committee candidate data");
    }
}
