...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::ProcessAllAndPropose {
            ...
            let (config, keys_file) = derive_paths(&state_dir, config, keys_file);
            let message_dir = messages_dir.unwrap_or_else(|| state_dir.join("dkg-messages"));
            let config_content = load_config(&config)?;
            ...
            println!("  Messages directory: {:?}", message_dir);
            ...
            let messages = load_messages_from_dir(&message_dir)?;
