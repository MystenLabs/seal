...
mod service_tests {
    ...
    async fn test_fetch_key() {
        ...
        let package_id = ObjectID::from_str(
            "0x9709d4ee371488c2bc09f508e98e881bd1d5335e0805d7e6a99edd54a7027954",
        )
        .unwrap();

        let whitelist_id = ObjectID::from_str(
            "0xaae704d2280f2c3d24fc08972bb31f2ef1f1c968784935434c3296be5bfd9d5b",
        )
        .unwrap();

        let user_secret_key = Ed25519PrivateKey::from_bytes(&[
            16, 38, 58, 130, 194, 133, 180, 117, 252, 32, 106, 49, 97, 22, 170, 130, 33, 59, 81,
            63, 132, 11, 246, 227, 58, 130, 18, 208, 130, 124, 49, 12,
        ])
        .unwrap();
        let keypair = Ed25519KeyPair::from(user_secret_key);
        let user = SuiAddress::from_str(
        ...
        let signature =
            GenericSignature::Signature(Signature::new_secure(&msg_with_intent, &keypair));
