...
use seal_sdk::{
    aggregate_encrypted_responses, decrypt_seal_responses, genkey, seal_decrypt_object,
...
/// and decrypting the aggregated result to obtain the final user secret key.
async fn get_aggregated_key_from_committee(
    ...
    // G1Element for encryption, G2Element for verification key.
    ...
    // Verify encrypted signatures for each response.
    let verified_responses: Vec<(u16, FetchKeyResponse)> = responses_with_party_ids
        .into_iter()
    ...
    let aggregated_response = aggregate_encrypted_responses(threshold, verified_responses).unwrap();

    // The client decrypts it with ephemeral secret key.
    elgamal::decrypt(
        &eg_sk,
