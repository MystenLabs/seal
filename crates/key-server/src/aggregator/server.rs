...
use key_server::common::ClientSdkType;
...
async fn fetch_from_member(
    ...
    let git_version = response.headers().get("X-KeyServer-GitVersion");
...
mod tests {
    ...
    async fn test_version_validations() {
        ...
        // Test 3: Aggregator rejects key server response if version is too old
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/v1/fetch_key"))
            .respond_with(
                ResponseTemplate::new(200)
                    .insert_header("X-KeyServer-Version", "0.5.13") // Too old
